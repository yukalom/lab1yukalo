...
export async function seedIfNeeded() {
    let row = await get("SELECT COUNT(*) AS count FROM softwareProducts");
    let count = Number(row?.count ?? 0);
    if (count > 0) {
        console.log("Seed skipped: table softwareProducts is not empty");
        return;
    }
    row = await get("SELECT COUNT(*) AS count FROM user");
    count = Number(row?.count ?? 0);
    if (count > 0) {
        console.log("Seed skipped: table user is not empty");
        return;
    }
    row = await get("SELECT COUNT(*) AS count FROM license");
    count = Number(row?.count ?? 0);
    if (count > 0) {
        console.log("Seed skipped: table license is not empty");
        return;
    }
    row = await get("SELECT COUNT(*) AS count FROM request");
    count = Number(row?.count ?? 0);
    if (count > 0) {
