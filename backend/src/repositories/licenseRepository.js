...
export async function getById(id) {
    const row = await get("SELECT * FROM license WHERE id = ?", [id]);
...
export async function getAll(query) {
    ...
    const countRow = await get(`SELECT COUNT(*) AS count FROM license${whereSql}`, params);
    const rows = await all(`SELECT * FROM license${whereSql} ORDER BY ${sortColumn} ${sortDirection} LIMIT ? OFFSET ?`, [...params, limit, offset]);
    return {
