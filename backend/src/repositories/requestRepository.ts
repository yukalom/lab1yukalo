...
type RequestSortBy = NonNullable<ListItemsQuery["sortBy"]>;

const sortColumnMap: Record<RequestSortBy, string> = {
  id: "id",
...
export async function getById(id: number): Promise<requestDto | null> {
  const row = await get<requestRow>("SELECT * FROM request WHERE id = ?", [id]);
  return row ? mapRowToDto(row) : null;
}

export async function update(dto: requestUpdateDto): Promise<requestDto | null> {
  const result = await run(
    `UPDATE request SET request_date = ?, software_id = ?, user_id = ? WHERE id = ?`,
    [dto.request_date, dto.software_id, dto.user_id, dto.id]
  );
...
export async function getAll(query: ListItemsQuery): Promise<listRequest> {
  ...
  const sortBy: RequestSortBy = query.sortBy ?? "user_id";
  ...
  const rows = await all<requestRow>(
    `SELECT * FROM request${whereSql} ORDER BY ${sortColumn} ${sortDirection} LIMIT ? OFFSET ?`,
    [...params, limit, offset]
  );
...
export async function create(dto: requestCreateDto): Promise<requestDto> {
  const result = await run(
    ...
    [dto.software_id, dto.user_id, dto.request_date]
  );
